      ...
      <n-form ref="formRef" :model="model" :rules="rules">
        <div class="fw-bold">
          ...
          <div class="row">
            <div class="col-6">
              ...
              >
                <n-input v-model:value="props.editData.type" :disabled="true" />
              </n-form-item>
            </div>
            <div class="col-6">
              <n-form-item
                ...
                label-style="font-weight: 700; font-size:16px"
              >
                <n-input v-model:value="props.editData.city" :disabled="true" />
              </n-form-item>
            </div>
          </div>
        ...
        <div class="row">
          <div class="col text-center">
            <n-button type="info" class="me-3" @click="submit">更新</n-button>
            <n-button type="error" @click="closeEditModal">取消</n-button>
          </div>
        </div>
      </n-form>
    </n-card>
...
async function editStatus(parms) {
  try {
    isLoading.value = true;
    const url = `${import.meta.env.VITE_NODE_URL}/mail/status`;
    console.log(parms);
    await axios.put(url, parms);
  } catch (error) {
...
function dataObj() {
  ...
  const newObj = {
    ...
    title: props.editData.type,
    min: model.value.time,
    email: model.value.textareaValue
      .split(",")
      .filter((item) => item)
...
const emit = defineEmits(["closeEditModal", "submit"]);
...
watch(
  ...
  () => {
    oldData.value = {
      city: props.editData.city,
      title: props.editData.type,
      min: parseInt(props.editData.min),
      mail: props.editData.mail,
    };
    model.value.time = parseInt(props.editData.min);
    model.value.textareaValue = editEmail(props.editData.mail);
  },
  { deep: true }
);
</script>
