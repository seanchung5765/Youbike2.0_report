<template>
  <div class="contanier">
    <div class="row mx-0">
      <h1 class="report-h1 fw-bold">電量緊急派工</h1>
    </div>
    <n-tabs v-model:value="valueRef" type="card" :tab-style="style" size="large">
      <n-tab-pane v-for="panel in panelsRef" :key="panel" :name="panel">
        <SendComponent2 :maildata="maildata" :city="valueRef" type="電量緊急派工" />
      </n-tab-pane>
    </n-tabs>
  </div>
</template>
...
async function getData() {
  try {
    ...
    totalData.value = res.data.data.filter((i) => i.type_name === "電量緊急派工");
  } catch (error) {
    console.log(error);
  }
}
